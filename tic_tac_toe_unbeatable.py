...
def testing_Winning_Moves(b, mark, i):
    bCopy = get_Board_Copy(b)
    bCopy[i] = mark
    return check_for_Win(bCopy, mark)
    
    
def testing_Fork_Moves(b, mark, i):
    bCopy = get_Board_Copy(b)
    bCopy[i] = mark
    winningMoves = 0
    for j in range(0, 9):
        if testing_Winning_Moves(bCopy, mark, j) and bCopy[j] == ' ':
            winningMoves += 1
    return winningMoves >= 2

def get_Computer_Moves(b):
    for i in range(0, 9):
        if b[i] == ' ' and testing_Winning_Moves(b, 'X', i):
            return i
    for i in range(0, 9):
        if b[i] == ' ' and testing_Winning_Moves(b, '0', i):
            return i
    # Check computer fork opportunities
    for i in range(0, 9):
        if b[i] == ' ' and testing_Fork_Moves(b, 'X', i):
            return i
    # Check player fork opportunities, incl. two forks
    playerForks = 0
    for i in range(0, 9):
        if b[i] == ' ' and testing_Fork_Moves(b, '0', i):
            playerForks += 1
            tempMove = i
    if playerForks == 1:
        return tempMove
...
while Playing:
    ...
    while InGame:
        if playerMarker == '0':
            ...
            move = get_Computer_Moves(board)
        board[move] = playerMarker
        if check_for_Win(board, playerMarker):
            InGame = False
            displayBoard(board)
            if playerMarker == '0':
                print('Noughts won!')
